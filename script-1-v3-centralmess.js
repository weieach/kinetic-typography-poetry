...
const createPoemLineMeshes = () => {
  ...
  for (let i = 0; i < poemChiLine.length; i++) {
    ...
    textGeometry.center();
    text.position.x = (Math.random() - 0.5) * 1;
    text.position.y = (Math.random() - 0.5) * 1;
    text.position.z = 0;
    scene.add(text);
  }
};
...
const tick = () => {
  ...
  for (let i = 0; i < poemLineArr.length; i++) {
    poemLineArr[i].position.z += Math.random() * 0.001;
    poemLineArr[i].position.x += Math.random() * 0.001;
    // poemLineArr[i].rotation.x += Math.random() * 0.001;
  }
  if(frameCount %30 == 0){
      for (let i = 0; i < poemLineArr.length; i++) {
